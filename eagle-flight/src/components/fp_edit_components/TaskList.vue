<template>
    <v-card color="primary" width="500px" height="300px">
        ...
        <v-card color="transparent" width="100%" height="85%" class="d-flex justify-center align-center">
            <v-list bg-color="transparent" width=96% height=96%>
                <v-card v-for="task in tasks" :key="task.id" class="d-flex align-center mb-3"
                    :color="selectedTask == task ? 'primary' : 'background'" height="40px">
                    <v-card-title class="d-flex w-100">
                        <span class="text-overline">{{ task.name }}</span>
                        <v-spacer></v-spacer>
                        <v-icon :color="selectedTask == task ? 'background' : 'primary'" class ="custom-icon"
                            @click="selectTask(task)">mdi-pencil</v-icon>
                        <v-icon :color="selectedTask == task ? 'background' : 'primary'" class="custom-icon"
                            @click="deleteTask(task)">mdi-delete</v-icon>
                    </v-card-title>
                </v-card>
            </v-list>
        </v-card>
...
let tasks = computed(() => taskStore.allTasks)
