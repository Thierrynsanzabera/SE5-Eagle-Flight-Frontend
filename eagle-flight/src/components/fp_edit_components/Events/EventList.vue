            ...
            <v-list bg-color="transparent" width=96% max-height=96%>
                <v-card v-for="event in events" :key="event.id" class="d-flex align-center mb-3"
                    ...
                    <v-card-title class="d-flex w-100">
                        ...
                        <v-spacer></v-spacer>
                        <v-icon :color="selectedEvent == event ? 'background' : 'primary'" @click="openEditEvent(event)">mdi-pencil-circle</v-icon>
                        <v-icon :color="selectedEvent == event ? 'background' : 'primary'" class="delete-icon"@click="deleteEvent(event)">mdi-delete-circle</v-icon>
                    </v-card-title>
                </v-card>
            </v-list>
        </v-card>
